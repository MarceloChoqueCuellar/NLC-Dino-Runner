...
from nlc_dino_runner.utils.constants import (
    ...
    JUMPING_SHIELD,
    DEFAULT_TYPE,
    SHIELD_TYPE, HAMMER_TYPE, RUNNING_HAMMER, JUMPING_HAMMER, DUCKING_HAMMER, LIGHT_MODE
)
    ...
    def check_invincibility(self, screen):
        if self.shield:
            ...
            if time_to_show < 0:
                ...
                if self.show_text:
                    text, text_rect = get_centered_message(f"Shield enabled for time: {time_to_show}", width=500,
                                                           height=40, size=20, color = color)
